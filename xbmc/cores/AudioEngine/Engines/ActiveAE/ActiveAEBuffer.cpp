  ...
  else if (m_procSample || !m_freeSamples.empty())
  {
    ...
    if (hasInput || skipInput || m_drain || m_changeResampler)
    {
      ...
      if (in)
      {
        ...
        m_lastSamplePts +=
            static_cast<int64_t>(in->pkt->nb_samples - in->pkt_start_offset) * 1000 / in->pkt->config.sample_rate;
      }
      ...
      m_procSample->timestamp = m_lastSamplePts - static_cast<int64_t>(bufferedSamples) * 1000 / m_format.m_sampleRate;
  ...
  else if (m_procSample || !m_freeSamples.empty())
  {
    ...
    if (hasInput || skipInput || m_drain || m_changeFilter)
    {
      ...
      if (in)
      {
        ...
        // pts of last sample we added to the buffer
        m_lastSamplePts += static_cast<int64_t>(in->pkt->nb_samples - in->pkt_start_offset) * 1000 / m_format.m_sampleRate;
      }
